...
declare
  table_name text;
begin
  foreach table_name in array array['participants', 'events', 'enrollments', 'payments', 'revenue_audit_log', 'expenses', 'reviews', 'promo_codes', 'promo_code_usages', 'event_price_tiers', 'site_media']
  loop
    if not exists (
      select 1 from pg_policies
